...
def compare_layers(words_dict,sentences_list,num_of_iter=-1):
    num_of_states=13
    layers_sim=np.zeros((num_of_states,num_of_states))
    num_of_sentences=0
    for word in words_dict:
        for mask_loc in words_dict[word]:
            current_sentences=[mask_word(sentences_list[i],int(mask_loc)) for i in words_dict[word][mask_loc]]
...
if __name__ == "__main__":
    word_to_sen=utils.read_dict_from_json('word_to_sen_dict')
    sentences_list=utils.read_dict_from_json('sentences_list')
